...
from density_estimate.analyse import analyse

st.set_page_config(page_title='SLG Raman Analysis', layout='wide')
...
if app_mode == "Home":
    st.write("Welcome to the SLG Raman analysis app. Use the sidebar to navigate through the application.")

elif app_mode == "Find electron density from spectrum":
    st.header("Doping analysis")
    uploaded_file = st.file_uploader("Upload your spectrum (CSV format)", type="csv")
    model_name = st.text_input("Enter a name for your dataset", value="cancer_model")
